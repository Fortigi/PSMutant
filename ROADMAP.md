...
#54 run result         ---> #11 survivors in CI, #6 per-file scores
                            the object CI reads carries a verdict without its reason, so a
                            failing gate prints a percentage and nothing else.
```
...
| 2 | **#11** | surface survivors in CI instead of in a log. Only possible once the result carries them |
| 3 | **#39** | an interrupted run loses everything: the report is written after the last mutant |
| 4 | **#124** | a suspended run hangs forever -- mutants are bounded, the run is not |
...
Measure before choosing a mechanism. Each of these multiplies by the mutant count, so a
several-hundred-mutant run is where they show.

| Order | Issue | Why here |
|---|---|---|
| 1 | **#102** | every mutant re-imports Pester into a fresh runspace: a flat 130 ms floor |
| 2 | **#101** | each mutant reads and writes the whole file twice; past 16 KB a write costs 40 ms on Windows |
| 3 | **#107** | every recheck round pays for coverage instrumentation that cannot change what it rechecks |
| 4 | **#62** | the timeout is derived from a *solo* baseline. Blocks #1 |
| 5 | **#1** | parallel evaluation. Needs #53 and #62 first, and is bigger than "add a runspace pool" |
...
- **#136** -- the analyzer script reports findings and exits 0, so running it by hand checks
  nothing. The sibling has the same gap as PSComplexity#85.
