...
function Get-PSMutationBinaryCandidate {
    ...
    foreach ($n in $nodes) {
        ...
        New-PSMutationCandidate $ext $File $ext.Text $to 'BinaryOperator' "$($ext.Text) -> $to"
    }
}
...
function Get-PSMutationBooleanCandidate {
    ...
    foreach ($n in $nodes) {
        ...
        New-PSMutationCandidate $n.Extent $File $n.Extent.Text $flip 'BooleanLiteral' "$($n.Extent.Text) -> $flip"
    }
}
...
function Get-PSMutationNumberCandidate {
    ...
    foreach ($n in $nodes) {
        ...
        New-PSMutationCandidate $n.Extent $File $n.Extent.Text $to 'NumberLiteral' "$($n.Value) -> $to"
    }
}
...
function Get-PSMutationStringCandidate {
    ...
    foreach ($n in $nodes) {
        ...
        if (Test-PSMutationInLoop -Extent $n.Extent -Ranges $Ranges) { continue }
        New-PSMutationCandidate $n.Extent $File $n.Extent.Text "''" 'StringLiteral' "string -> ''"
    }
}
...
function Get-PSMutationNegationCandidate {
    ...
    foreach ($n in $nodes) {
        ...
        if (Test-PSMutationInLoop -Extent $n.Extent -Ranges $Ranges) { continue }
        New-PSMutationCandidate $n.Extent $File $n.Extent.Text $n.Child.Extent.Text 'NegationRemoval' 'remove negation'
    }
}
